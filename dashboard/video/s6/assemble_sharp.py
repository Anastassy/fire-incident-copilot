...
parser.add_argument('--approved-video', type=Path, required=True, help='Approved original MP4: copy its audio unchanged')
...
SHOTS = [
 cap('D01',25,30,'overview',0,[66,64,1838,1000],'ONE OPERATOR. MANY SOURCES.'),
 cap('D02',30,33,'overview',4,[1236,172,642,700],'SENSORS / CAMERAS / RADIO','source'),
 cap('D03',33,38,'overview',3,[1236,260,642,478],'THE INCIDENT CONTINUES TO CHANGE'),
 cap('D04',38,43,'assessment',0,[116,602,1076,440],'COPILOT / AUTOMATIC ASSESSMENT'),
 cap('D05',43,47,'assessment',0,[116,716,1076,326],'EVIDENCE AND UNKNOWNS, SIDE BY SIDE'),
 cap('D06a',47,49,'power-evidence',0,[1420,220,485,540],'FOLLOW THE EVIDENCE','drawer',['SOURCE RECORD','Original reading','Device, time and provenance']),
 cap('D06b',49,53,'cameras',0,[1236,474,642,263],'CAMERA VIEWS / CHECK SOURCE FRESHNESS'),
 cap('D07',53,59,'connectivity',0,[1420,220,485,420],'FRESHNESS IS PART OF THE EVIDENCE','drawer',['CONNECTION LOST','SMK-B2-GRADAS / 03:04','A last reading is not a current reading.']),
 cap('D08a',59,63,'question',0,[116,1047,1076,130],'ASK INCIDENT COPILOT'),
 cap('D08b',63,64,'answer',0,[116,578,1065,349],'ASK INCIDENT COPILOT'),
 cap('D09a',64,66,'access',0,[1420,566,485,420],'CHECK THE SOURCE / ACCESS','drawer',['PERMISSION IS NOT PRESENCE','Access record / Gradas','A badge event cannot confirm occupancy.']),
 cap('D09b',66,70,'answer',0,[116,578,1065,349],'COPILOT / GROUNDED ANSWER'),
 cap('D10',70,74,'answer',0,[116,578,1065,349],'THE ANSWER EXPLAINS ITS LIMITS'),
 cap('D11',74,78,'radio',0,[1420,326,485,550],'LISTEN TO THE ORIGINAL RADIO','radio',['PALISADES ARCHIVE','Original transmission','Separate incident / source attached']),
 cap('D12',78,82,'radio',1,[1420,326,485,550],'KEEP INCIDENT PROVENANCE VISIBLE','radio',['SOURCE BOUNDARY','Palisades is a separate incident.','It cannot confirm conditions in Base2.']),
 cap('D13',82,90,'answer',0,[116,578,1065,349],'BRIEF COMMAND WITH EVIDENCE'),
]
...
(ROOT/'production/edit.json').write_text(json.dumps({'duration_s':120,'fps':30,'size':[1920,1080],'capture_note':'Native Chrome screenshots at full CSS resolution. No downsampled or duplicated tiles. Actual capture timing retained, then held to match approved edit windows. All answers are from the same existing G5 run; no generated UI text.','shots':SHOTS},indent=2))
...
def build(s):
    ...
    if 'slide' in s:
        ...
        filt=f"[0:v]crop={w}:{h}:{x}:{y},fps=30,scale={boxw}:{boxh}:force_original_aspect_ratio=decrease:flags=lanczos,pad={boxw}:{boxh}:(ow-iw)/2:(oh-ih)/2:color=0x141b1e,setsar=1[p];color=c=0x141b1e:s=1920x1080:r=30:d={length}[bg];[bg][p]overlay={left}:165:shortest=1,"
    ...
    run(args+['-an','-frames:v',str(round(length*30)),'-r','30','-c:v','libx264','-threads','2','-preset','veryfast','-crf','14','-pix_fmt','yuv420p','-video_track_timescale','15360',str(out)])
...
def main():
    ...
    # Copy the approved mix bit-for-bit; only the picture changes in this revision.
    out=ROOT/'firewatch-120s-clean.mp4'
    approved=args.approved_video.resolve()
    run(['ffmpeg','-hide_banner','-loglevel','error','-y','-i',str(video),'-i',str(approved),'-map','0:v','-map','1:a','-c','copy','-t','120','-movflags','+faststart',str(out)])
