...
export class LiveMedia {
 ...
 async start(sourcePlayer){
  if(!window.MediaSource){this.status('Браузер не поддерживает MediaSource');return;}
  const mime=this.item.kind==='audio'?'audio/mp4; codecs="mp4a.40.2"':'video/mp4; codecs="avc1.64001e"';
  if(!MediaSource.isTypeSupported(mime)){this.status('Кодек недоступен в этом браузере');return;}
  ...
  try{this.sb=this.ms.addSourceBuffer(mime);this.sb.mode='segments';this.sb.addEventListener('updateend',()=>this.pump());this.sb.addEventListener('error',()=>this.status('Ошибка декодирования'));this.ms.duration=this.run.duration_ms/1000+.1;
   ...
   const reader=response.body.getReader();this.status('Ожидание кадров');
 ...
 pump(){if(!this.sb||this.sb.updating||this.ms.readyState!=='open')return;
  if(this.queue.length){try{this.sb.appendBuffer(this.queue.shift());}catch(error){this.failed=true;this.status('Ошибка буфера: '+error.name);}return;}
 ...
 sync(simMs,playing,speed=1){
  ...
  if(this.item.kind==='audio'&&playing&&speed!==1){this.el.pause();this.status('Для прослушивания выберите 1×');return;}
  if(!playing){this.el.pause();let last=null;for(let i=0;i<this.el.buffered.length;i++){if(this.el.buffered.start(i)<=target)last=Math.min(target,this.el.buffered.end(i)-.06);}if(last!=null&&Math.abs(this.el.currentTime-last)>.3)this.el.currentTime=Math.max(0,last);this.status(this.el.buffered.length?'Пауза':'Ожидание Play');return;}
  if(this.buffered(target)){
   if(Math.abs(this.el.currentTime-target)>.5)this.el.currentTime=target;
   this.el.playbackRate=Math.min(16,speed);this.el.play().catch(()=>{if(!this.el.muted)this.status('Нажмите «Звук»');});
   this.status('Поток');
  }else{this.el.pause();this.status(this.eof&&simMs>=this.item.duration_ms?'Запись завершена':'Нет новых кадров');}
 }
 ...
 syncReview(){const r=this.review;if(!r.started&&this.buffered(r.start)){this.el.currentTime=r.start;r.started=true;this.el.play().catch(()=>this.status('Нажмите Play в плеере'));}if(r.started&&this.el.currentTime>=r.end)this.el.pause();}
