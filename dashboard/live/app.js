...
async function connectState(runId, scenarioId) {
  ...
  snapshot=null; observations=[]; renderState(); connection('state-connection',false,'Подключение…');
  try {
  ...
  }catch(error){if(ticket===epoch){connection('state-connection',false,'Нет подключения');toast('State API: '+error.message);renderState();}}
}
...
async function consumeState(ticket) {
  ...
  while(ticket===epoch&&!signal.aborted){
    try { await sse(runPath()+'/stream',{signal,lastEventId:snapshot.as_of.cursor,
      onOpen:()=>{stateConnected=true;connection('state-connection',true,'Источники · SSE');},
      onMessage:message=>{
        if(ticket!==epoch)return;
        const item=message.data;
        if(message.type==='stream_error')throw new Error(item.error?.message||'Нужна синхронизация');
      ...
      stateConnected=false;connection('state-connection',false,'Восстановление связи');
...
async function loadHistory(ticket=epoch) {
  ...
  }catch(error){if(ticket===epoch)toast('История: '+error.message);}
...
async function startMedia(ticket) {
  ...
  for(const item of streams){
    ...
    if(index!=null){$('camera-name-'+index).textContent=item.device_id.includes('GRADAS')?'Sala de Gradas':item.device_id.includes('GALLERY')?'Галерея у двери':item.device_id;}
  }
...
async function command(action,extra={}){
  ...
  try {const result=await api(runPath()+'/commands',{command_id:crypto.randomUUID(),expected_generation:snapshot.run.generation,action,...extra});
    ...
    scheduleRender();
  }catch(error){toast('Команда: '+error.message);if(error.status===409)await resync(ticket);}
...
function renderControls(){
  ...
  $('play').innerHTML=icon(playing?'pause':'play')+`<span>${playing?'Пауза':run?.status==='completed'?'Заново':'Запустить'}</span>`;
  $('run-status').textContent=run?statusLabel(run.status):'ОЖИДАНИЕ ПОДКЛЮЧЕНИЯ';
...
function sparkline(deviceId){
  ...
  return `<svg class="sparkline" viewBox="0 0 94 30" aria-label="Последние опубликованные измерения"><path d="${samples.map((v,i)=>`${i?'L':'M'}${i*94/(samples.length-1)},${27-(v-min)*24/range}`).join(' ')}" fill="none" stroke="currentColor" stroke-width="1.5"/></svg>`;
}
...
function renderState(){
  ...
  $('metrics').innerHTML=metricDevice(temp,base2?'Температура · Gradas':temp?.name||'Температура','temp')+metricDevice(smoke,base2?'Задымление · Gradas':smoke?.name||'Дымовой датчик','smoke')+metricDevice(gallery,base2?'Температура · галерея':gallery?.name||'Температура · источник 2','temp')+
    metric('Людей в здании',valueText(snapshot?.occupancy?.[0]?.count),'',snapshot?.occupancy?.[0]?.count==null?'Источник подсчёта отсутствует':statusLabel(snapshot.occupancy[0].availability),'people')+
    metric('Источники в сети',devices.filter(d=>d.availability==='fresh').length,`/ ${devices.length}`,'По состоянию источников','link')+
    metric('Радиопереговоры',transcripts.length,'реплик','Опубликовано в этом запуске','radio');
  if(snapshot){
    const scenario=scenarios.find(s=>s.scenario_id===snapshot.run.scenario_id);
    $('scenario-subtitle').textContent=scenario?.name||snapshot.run.scenario_id;
    $('building-label').textContent=base2?'BASE2 · VALENCIA':scenario?.building_id||snapshot.run.scenario_id;
    document.querySelectorAll('.camera-technical').forEach(el=>el.textContent=base2?'СИНТЕТИЧЕСКОЕ CCTV':'АРХИВНОЕ CCTV');
    ...
    banner.textContent=gaps.length?`${gaps.length} ${gaps.length===1?'источник требует':'источника требуют'} внимания · ${gaps.map(d=>d.name).join(' · ')}. Последние показания сохранены, новых данных нет.`:'';
    $('session-footer').textContent=`RUN ${snapshot.run.run_id.slice(0,8)} · G${snapshot.run.generation} · ${snapshot.system?.events?.dropped??'—'} пропущено · ${stateConnected?'поток подключён':'нет живой связи'}`;
  }
...
function renderMap(devices){
  ...
  $('floor-sensors').innerHTML=devices.filter(d=>positions[d.device_id]).map(d=>{const [x,y]=positions[d.device_id],color=d.availability==='fresh'?'#6fc9b5':d.availability==='stale'||d.availability==='disconnected'?'#e5a566':'#7a8287';return `<g class="sensor-dot" data-device="${esc(d.device_id)}" tabindex="0" role="button" aria-label="${esc(d.name)}"><title>${esc(d.name+' · '+statusLabel(d.availability))}</title><circle cx="${x}" cy="${y}" r="10" fill="${color}" opacity=".12"/><circle cx="${x}" cy="${y}" r="4" fill="${color}"/></g>`;}).join('');
}
function description(o){const d=o.data; if(o.kind==='measurement')return `${d.metric} · ${valueText(d.value,d.unit)}`;if(o.kind==='radio_transcript')return d.text||d.transcript||'Текст недоступен';if(o.kind==='camera')return 'Опубликован фрагмент видеозаписи';if(o.kind==='radio_audio')return 'Опубликован фрагмент радио';if(o.kind==='access')return d.description||d.event_type||d.action||'Событие считывателя пропуска';if(o.kind==='connectivity')return d.connected?'Соединение восстановлено':'Соединение потеряно';return JSON.stringify(d);}
...
function renderRadio(items){
  ...
  list.innerHTML=items.filter(o=>!selected||o.device_id===selected.device_id).sort(byTime).map(o=>`<button class="transcript" data-observation="${esc(o.evidence_id)}"><span class="transcript-time">${timecode(o.data.audio_start_sim_time_ms??timeOf(o))} ${icon('play')}</span><p>${esc(description(o))}</p></button>`).join('')||'<div class="empty-inline">Реплики появятся по мере поступления аудио.<br><span>Архивная запись · готовая машинная расшифровка</span></div>';
...
async function refreshAgent(){
  ...
  try{const next=await api('/api/agent/agent/v1/state?'+contextQuery(context));
    ...
    if(!agentSnapshot||next.snapshot_revision>=agentSnapshot.snapshot_revision){agentSnapshot=next;lastAgentError='';connection('agent-connection',true,'Агент · API');renderAgent();}
  }catch(error){if(ticket===agentEpoch){lastAgentError=error.message;connection('agent-connection',false,'Агент недоступен');renderAgent();}}
}
async function connectAgent(context){
  ...
  while(ticket===agentEpoch&&!signal.aborted){
    try{await sse('/api/agent/agent/v1/events?'+contextQuery(context),{signal,onOpen:refreshAgent,onMessage:m=>{
      ...
      if(m.data.type==='context.invalidated'){lastAgentError='Контекст сброшен на сервере. Укажите новое поколение в подключении.';agentSnapshot=null;renderAgent();return;}
      refreshAgent();
    }});}catch{if(signal.aborted||ticket!==agentEpoch)return;await refreshAgent();await sleep(2000);}
  }
}
function claimMarkup(claim){const kinds={source_report:'Доклад источника',measurement:'Измерение',inference:'Вывод агента',absence_in_checked_data:'Не найдено в проверенном'};return `<div class="claim"><span class="claim-kind">${esc(kinds[claim.kind]||claim.kind)}</span><p>${esc(claim.text)}</p><div>${(claim.evidence_ids||[]).map((id,i)=>`<button class="evidence-link" data-evidence="${esc(id)}">${icon('link')} Источник ${i+1}</button>`).join('')}</div></div>`;}
function traceMarkup(entity){const c=entity.coverage;return `<div class="unknowns">${(entity.unknowns||[]).length?'<strong>Пока неизвестно</strong>':''}${(entity.unknowns||[]).map(x=>`<p>${esc(x)}</p>`).join('')}</div><details class="trace"><summary>Область проверки и запросы ${icon('arrow')}</summary>${c?`<p>${timecode(c.checked_from_ms)}–${timecode(c.checked_until_ms)} · ${esc({complete:'Полная',partial:'Частичная',unknown:'Полнота неизвестна'}[c.completeness])}</p>${c.limitations.map(x=>`<p>${esc(x)}</p>`).join('')}`:''}<pre>${esc(JSON.stringify(entity.queries||[],null,2))}</pre></details>`;}
function renderAgent(){
  ...
  const welcome='<div class="copilot-welcome"><span class="eyebrow">ДОКАЗАТЕЛЬСТВА ПЕРЕД ВЫВОДАМИ</span><h3>Что известно.<br>Чего пока не знаем.</h3><p>Вопрос запускает проверку источников в контексте агента. Каждый вывод открывается до исходного сообщения.</p></div>';
  let content='';
  if(agentTab==='checks')content=cards.map(c=>`<article class="check-card"><div class="card-status-line"><span class="status-tag">${esc(statusLabel(c.assessment))}</span><span>v${c.revision}</span></div><h3>${esc(c.statement)}</h3>${c.claims.map(claimMarkup).join('')}${traceMarkup(c)}<details class="trace"><summary>Состояние проверки</summary><p>Жизненный цикл: ${esc(statusLabel(c.lifecycle))}</p><p>Публикация: ${esc(c.publication_status)}. Pending может означать, что публикация не используется.</p><p>Статус платформы: ${esc(c.platform_status||'не назначен')}</p>${c.closure_reason?`<p>${esc(c.closure_reason)}</p>`:''}<button class="evidence-link" data-card="${esc(c.hypothesis_id)}">Полная карточка API</button></details></article>`).join('')||'<div class="empty-inline">В этом контексте ещё нет проверок.</div>';
  else content=[...answers].sort((a,b)=>b.created_at.localeCompare(a.created_at)).map(a=>`<article class="answer-card"><div class="answer-question">${esc(a.question)}</div><div class="answer-status ${esc(a.status)}">${icon(a.status==='ready'?'check':'spark')}${esc(statusLabel(a.status))}${['queued','running'].includes(a.status)?`<button class="evidence-link" data-cancel="${esc(a.request_id)}">Отменить</button>`:''}</div>${a.claims.map(claimMarkup).join('')}${a.error?`<p class="unknowns">${esc(a.error.message)}</p>`:''}${traceMarkup(a)}${a.status==='error'?`<button class="evidence-link" data-retry="${esc(a.request_id)}">Повторить вопрос</button>`:''}</article>`).join('')||welcome;
  if(pendingQuestion)content=`<div class="loading-text">${esc(pendingQuestion.question)} · отправляем…</div>`+content;
  if(uncertainQuestion)content='<div class="empty-inline">Ответ на отправку не получен. <button class="evidence-link" data-resend="true">Повторить с тем же ID</button></div>'+content;
...
async function askQuestion(text,reuseBody){
  ...
  const body=reuseBody||{context,client_request_id:crypto.randomUUID(),question:text.trim(),language:'ru'};
  ...
  try{const answer=await api('/api/agent/agent/v1/questions',body);
    ...
    await refreshAgent();
  }catch(error){if(ticket===agentEpoch){if(!error.status||error.status>=500)uncertainQuestion=body;toast('Вопрос: '+error.message);}}
  finally{if(ticket===agentEpoch){pendingQuestion=null;renderAgent();}}
}
async function seedAgent(action){try{const context=await api('/api/agent-demo',{action});if(!contextEqual(agentContext,context))connectAgent(context);else await refreshAgent();}catch(error){toast('Тест агента: '+error.message);}}
...
function openDrawer(title,body,raw){closeEvidence();$('evidence-drawer').hidden=false;$('drawer-backdrop').hidden=false;$('evidence-body').innerHTML=`<h2>${esc(title)}</h2>${body}`;$('evidence-raw').innerHTML=raw?`<details><summary>Исходные данные JSON</summary><pre>${esc(JSON.stringify(raw,null,2))}</pre></details>`:'';}
function openObservation(id){
  ...
  if(o.kind==='radio_transcript'){
    ...
    if(source&&Number.isFinite(start)&&Number.isFinite(end)){
      $('evidence-body').insertAdjacentHTML('beforeend',`<button class="primary-button" id="review-fragment">${icon('play')} Прослушать ${timecode(start)}–${timecode(end)}</button><p class="evidence-meta">Машинная расшифровка из пакета. Аудио Provided by Broadcastify · CC-BY-3.0-US.</p>`);
      $('review-fragment').onclick=()=>{const ticket=drawerEpoch;
        ...
        if(!original)return toast('Поток уже сменился. Откройте реплику заново.');
...
async function openAgentEvidence(id){
  ...
  try{const e=await api('/api/agent/agent/v1/evidence/'+encodeURIComponent(id)+'?'+contextQuery(context));if(ticket!==drawerEpoch||!contextEqual(context,agentContext))return;
    ...
    const a=e.audio;if(a?.availability==='available'&&a.playback_url){const url=new URL(a.playback_url,location.href);if(!['http:','https:'].includes(url.protocol))throw new Error('Неподдерживаемый адрес аудио');const el=$('review-audio');el.hidden=false;el.src=url.href;el.onloadedmetadata=()=>el.currentTime=a.start_ms/1000;el.ontimeupdate=()=>{if(el.currentTime>=a.end_ms/1000)el.pause();};}
    else $('evidence-body').insertAdjacentHTML('beforeend',`<p class="unknowns">Аудио: ${esc(a?.availability||'missing')}. Сервис агента пока не возвращает запись для этого основания.</p>`);
  }catch(error){if(ticket===drawerEpoch)$('evidence-body').textContent='Источник недоступен: '+error.message;}
}
function openDevice(id){const d=snapshot?.devices.find(x=>x.device_id===id);if(!d)return;openDrawer(d.name,`<div class="evidence-meta">${esc(d.device_id)} · ${esc(d.room_id)} · ${esc(statusLabel(d.availability))}</div>${d.readings.map(r=>`<p>${esc(r.metric)}: <strong>${esc(valueText(r.value,r.unit))}</strong><br><span class="evidence-meta">${esc(statusLabel(r.availability))} · последнее измерение ${r.observed_sim_time_ms==null?'не поступало':timecode(r.observed_sim_time_ms)}</span></p>`).join('')}${sparkline(id)}`,d);}
...
document.addEventListener('click',async event=>{
  ...
  if(target.dataset.system)openDrawer('Система и источники',`<p>Сведения State API. Отсутствие данных не означает норму.</p>`,{system:snapshot?.system,devices:snapshot?.devices,access:snapshot?.access,occupancy:snapshot?.occupancy});
  ...
  if(target.dataset.expand!=null) $('camera-'+target.dataset.expand).closest('article').requestFullscreen?.().catch(()=>toast('Полный экран недоступен'));
...
$('coverage-track').title='Перемотать сценарий: изменится поколение этого запуска';
...
$('sound-toggle').onclick=()=>{sound=!sound;const audio=$('radio-audio');audio.muted=!sound;if(sound)audio.play().catch(()=>{});$('sound-toggle').classList.toggle('enabled',sound);$('sound-toggle').querySelector('span').textContent=sound?'Звук вкл.':'Звук выкл.';};
...
$('settings-connect').onclick=()=>{const context={demo_context_id:$('agent-context-input').value.trim(),generation:Number($('agent-generation-input').value),subject_id:$('agent-subject-input').value.trim()};if(!context.demo_context_id||!context.subject_id||!Number.isInteger(context.generation)||context.generation<0){toast('Укажите полный контекст агента');return;}$('settings-dialog').close();connectState($('run-id-input').value.trim(),$('scenario-select').value);connectAgent(context);};
...
async function boot(){
  ...
  try{config=await api('/api/config');
    try{scenarios=(await api('/api/state/scenarios')).items;}catch(error){toast('Каталог State API: '+error.message);}
    $('scenario-select').innerHTML=scenarios.map(s=>`<option value="${esc(s.scenario_id)}">${esc(s.name)}</option>`).join('');$('scenario-select').value=config.scenario;
    const fixture=config.agent.engine==='FixtureEngine';
    $('agent-mode').textContent=fixture?'Тестовый обработчик · FixtureEngine':config.agent.engine||'Не подключён';
    $('agent-context-note').textContent=fixture?'Отдельный тест API агента. Эти ответы и проверки не анализируют поток Base2.':'Ответы и проверки из подключённого контекста агента. Источники и время проверки указаны в каждом выводе.';
    $('agent-demo-tools').hidden=!(fixture&&config.demo_agent);
    $('endpoint-info').textContent=`State: ${config.state_url}\nAgent: ${config.agent_url}\nData Platform: ${config.platform_configured?config.platform_url:'ещё не подключена'}\nИмпорт агента: ${JSON.stringify(config.agent.platform||{})}`;
    ...
    if(config.state_configured)connectState(new URL(location.href).searchParams.get('run'),config.scenario);else toast('Укажите State API credentials при запуске gateway.');
  }catch(error){toast('Подключение: '+error.message);}
}
boot();
